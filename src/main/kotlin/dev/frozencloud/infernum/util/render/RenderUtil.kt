...
import dev.frozencloud.infernum.mixin.accessors.AccessorBeaconBeam
import dev.frozencloud.infernum.util.unaryMinus
...
fun WorldRenderEvent.Extract.drawCylinder(m: Vec3, r: Float, h: Float, color: Color, segments: Int = 32, thickness: Float = 5f, phase: PhaseType = PhaseType.NO_PHASE) {
    val batch = RenderConsumer.lines[phase.ordinal]
    ...
    for (i in 0 until segments) {
        ...
        val x1 = (r * cos(angle1)).toFloat()
        val z1 = (r * sin(angle1)).toFloat()
        val x2 = (r * cos(angle2)).toFloat()
        val z2 = (r * sin(angle2)).toFloat()

        val p1Top = m.add(x1.toDouble(), h.toDouble(), z1.toDouble())
        val p2Top = m.add(x2.toDouble(), h.toDouble(), z2.toDouble())
        val p1Bottom = m.add(x1.toDouble(), 0.0, z1.toDouble())
        val p2Bottom = m.add(x2.toDouble(), 0.0, z2.toDouble())
