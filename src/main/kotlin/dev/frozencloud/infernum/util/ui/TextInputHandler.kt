package dev.frozencloud.infernum.util.ui
...
//        NanoVGHelper.resetScissor()
    ...
    fun onMouseClicked(mouseButtonEvent: MouseButtonEvent): Boolean {
        ...
        when (clickCount) {
            1 -> {
                caretFromMouse(mouseButtonEvent.x.toFloat())
                clearSelection()
            }
    ...
    private fun insert(string: String) {
        ...
        val newText =
            text.substringSafe(0, start) +
                    string +
                    text.substringSafe(end, text.length)
    ...
    private fun caretFromMouse(mouseX: Float) {
        ...
        for (i in text.indices) {
            val w = textWidth(text[i].toString())
            if (currWidth + w / 2f > mx) break
            currWidth += w
            newCaret = i + 1
        }
    ...
    private fun textWidth(text: String): Float = NanoVGHelper.textWidth(text, height - 2, NanoVGHelper.defaultFont)
    ...
    private fun String.removeRangeSafe(from: Int, to: Int): String =
        removeRange(min(from.coerceAtLeast(0), to), max(to, from.coerceAtLeast(0)))
