...
import dev.frozencloud.infernum.util.ui.MouseUtil
...
class ModuleDropdownComponent(val module: Module) {
    ...
    fun draw(x: Float, y: Float): Float {
        ...
        val color = when {
            colorAnim.isAnimating() -> colorAnim.get(Colors.DisabledBackground, Colors.InfernumAccentDark, !module.enabled)
            else -> if (module.enabled) Colors.InfernumAccentDark else Colors.DisabledBackground
        }.rgba

        val descTextColor = when {
            colorAnim.isAnimating() -> colorAnim.get(Colors.TextMuted, Colors.TextSecondary, !module.enabled)
            else -> if (module.enabled) Colors.TextSecondary else Colors.TextMuted
        }.rgba
        ...
        NanoVGHelper.text(NanoVGHelper.defaultFont, module.description, x + textWidth + PADDING + 8f, y + 16f, 18f, descTextColor)
