...
import dev.frozencloud.infernum.util.ui.animations.LinearAnimation
...
class NumberSetting<E>(
    ...
    override fun render(x: Float, y: Float, right: Float, mouseX: Float, mouseY: Float): Float {
        ...
        if (right - SLIDER_WIDTH + sliderPercentage * (SLIDER_WIDTH) > right - SLIDER_WIDTH)
            NanoVGHelper.roundedRect(SLIDER_X, SLIDER_Y, sliderAnim.get(prevLocation, sliderPercentage, false) * SLIDER_WIDTH, SLIDER_HEIGHT, 3f, Colors.InfernumAccent.rgba)
