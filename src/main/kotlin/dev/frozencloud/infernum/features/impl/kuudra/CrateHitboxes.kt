...
import dev.frozencloud.infernum.util.cratePos
...
import dev.frozencloud.infernum.util.render.drawBeaconBeam
...
object CrateHitboxes : Module(
    ...
    init {
        Scheduler.addTask(10, false, { KuudraUtil.inKuudra }) {
    ...
    fun onRenderWorldExtract(event: WorldRenderEvent.Extract) {
        ...
        if (giantBox) {
            KuudraUtil.crates.forEach {
                val phase = if (giantPhase) PhaseType.PHASE else PhaseType.NO_PHASE
                event.drawOutlinedBox(it.renderBoundingBox, giantColor, 3f, if (giantPhase) PhaseType.PHASE else PhaseType.NO_PHASE)

                when (giantRenderType) {
                    "Outlined" -> event.drawOutlinedBox(it.renderBoundingBox, giantColor, phase = phase)
                    "Filled" -> event.drawFilledBox(it.renderBoundingBox, giantColor, phase = phase)
                    "Filled outline" -> {
                        event.drawOutlinedBox(it.renderBoundingBox, giantColor, phase = phase)
                        event.drawFilledBox(it.renderBoundingBox, giantColor.withAlpha(0.25f), phase = phase)
                    }
                }
            }
        }
        ...
        if (crateBox) {
            crateBoxes.forEach {
                when (crateBoxRenderType) {
                    "Outlined" -> event.drawOutlinedBox(it.key, it.value, phase = PhaseType.PHASE)
                    "Filled" -> event.drawFilledBox(it.key, it.value, phase = PhaseType.PHASE)
                    "Filled outline" -> {
                        event.drawOutlinedBox(it.key, it.value, phase = PhaseType.PHASE)
                        event.drawFilledBox(it.key, it.value.withAlpha(0.25f), phase = PhaseType.PHASE)
                    }
                }
            }
        }
