...
import dev.frozencloud.infernum.util.next
...
import dev.frozencloud.infernum.util.ui.animations.EaseInOutAnimation
...
class IslandsDropdownComponent(var current: Island) {
    ...
    fun render(x: Float, y: Float) {
        ...
        NanoVGHelper.roundedRect(x, y, WIDTH, HEIGHT,8f, Colors.InfernumAccent.rgba)
        ...
        if (extended || anim.isAnimating()) {
            ...
            Island.entries.forEachIndexed { index, island ->
                NanoVGHelper.text(NanoVGHelper.defaultFont, island.name, x + 38f, y + HEIGHT / 2f - 8f + HEIGHT * (index + 1), 16f, if (island == current) Colors.InfernumAccent.rgba else Colors.TextPrimary.rgba)
                if (index != 0)
